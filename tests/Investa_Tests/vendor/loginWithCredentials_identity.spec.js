...
export async function investa_loginWithPassword_vendor_identity(page, baseURL, username, password, testName) { //uses /identity/ instead of /auth/
    ...
    const domainApi = baseURL + "/identity/api/login/domain"
    try {
        await page.locator('//*[@id="app"]/main/section/div/div/div/form/div[2]/button').click();
        const response = await page.waitForResponse(res => res.url().startsWith(domainApi));
