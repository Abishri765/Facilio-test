...
export async function loginWithValidUsername_stage2Maintenace(page, baseURL, username, password, testname) {
  ...
  if (responseCode != 0) {
    ...
    try {
      ...
      if (responseCodePassword != 0) {
        ...
        let messagePrint = `${testname} - Authentication - ${message}`;
        await sendAlertMessage(messagePrint);
        await toPlaywrightReport(messagePrint);
